...
% \(c_n=c_{-n}^*\) for symmetry, and \(c_0\) depends on the first order of modulation, \(c_0=\frac{eE_R}{m\omega_x^2}\). Then we can expand it to the second order, and we can simplify \(\bar{x}(t)=c_0+2\Re{\qty(c_{-1}e^{-i\omega_mt})}\), substitute it back to Eq.~\ref{eq:parametric_excitation}, we can easily get the second-order solution, 
% \begin{equation}\label{eq:parametric_solution}
%   \norm{c_{-1}}^2=\frac{2c_0^2h^2}{\qty(f^2-1)+4f^2d^2},\quad \tan(\arg(c_{-1}))=\frac{2fd}{f^2-1}
% \end{equation}
% where \(f=\frac{\omega_m}{\omega_x},\, d=\frac{\xi}{\omega_x}\) are normalized frequency and damping factor. The amplitude of \(c_{-1}\) is linearly proportional to \(c_0\), and thus \(E_R\). Obviously, there is a \(\pi\) phase shift when scanning the modulation frequency across \(\omega_x\), which indicate the resonant frequency, and it can be further assisted with the peak value of \(c_{-1}\) during a frequency scan. The sensitivity is determined by two factors, (a) the modulation factor \(h\), (b) the detuning of the modulation frequency to the secular frequency(note that there is a correction of secular frequency due to the modulation which is not shown here, refer to Ref.~\inlinecite{nadlinger2021micromotion} for the details, but the underlying physcis holds the same.). 

% \begin{figure}
%   \centering
%   \includegraphics[width=0.6\linewidth]{parametric_excitation.pdf}
%   \caption[Protocol of parametric excitation]{\label{fig:parametrix_excitation}The schematic protocol to minimize excess micromotion using parametric excitation technique. The yellow indicates the blade-like electrodes, and the black and red arrows indicate the principal axes in radial plane, and the purple arrows show the procedure of minimization starting from \(A\).}
% \end{figure}

% In experiment, the amplitude modulation of RF voltage can be realized by an analog modulation of signal generator or a frequency mixer and RF power splitters, the modulation signal should be synchronized with the fluorescence to demodulate the correlation, in practice it can be realized via a TAC device, e.g., PicoHarp, or otherwise we can record the arrival time of each photons and demodulate the correlation digitally. Suppose \(\vv{AB}\) and \(\vv{BC}\) are two independent non-parallel compensation voltage vectors and we rename them as comp \(1\) and comp \(2\), the main operating procedure shown in Fig.~\ref{fig:parametrix_excitation} can be summarized as below,
% \begin{enumerate}[(1)]
%   \item Set appropriate modulation depth, and scan the modulation frequency, we can find the secular frequency for the phase shift or the peak value of correlation, suppose \(\omega_x,\,\omega_y\)
%   \item Fix the detuning \(\Delta\) to \(\omega_x\)
%   \item Tweak comp \(1\) and find the minimum of the parametric excitation correlation, here \(B\)
%   \item Change the modulation frequency to excite \(\omega_y\) with the same detuning \(\Delta\)(The detuning is actually not necessary to hold the same.)
%   \item Tweak comp \(2\) and find the minimum again, \(C\)
%   \item Repeat steps for \(\omega_x\) again, and find the minimum \(D\)
%   \item Now we can obtain the basis vector for \(\omega_y\), and we can combine comp \(1,\,2\) together along \(\vv{cy}=\vv{BD}\) to minimize parametric excitation, \(O\)
%   \item In principle, if the precision is good enough that we have already reached the optimal position. However, if higher precision is required, then we can iteratively update the compensation voltages, since we can always obtain a finer basis vectors from the last two minimization steps.
% \end{enumerate}

% Therefore, using such parametric excitation technique, we can realize high-precision minimization of excess micromotion of both axes and only one cooling beam is used and thus don't complicate the system configurations with additional lasers.
